...
@property (nonatomic, strong) BBCSMPSubtitleObserverManager *subtileObserverManager;
...
- (void)commonInitFromContext:(BBCSMPPlayerInitialisationContext*)context
{
    ...
    _subtileObserverManager = [[BBCSMPSubtitleObserverManager alloc] init];
...
- (void)setPlayerItemProvider:(id<BBCSMPItemProvider>)playerItemProvider
{
    ...
    [self.playerContext forceVolumeUpdate];
    [self.playerContext forceSubtitleStateUpdate];
...
- (BBCSMPError*)errorFromError:(NSError*)error
{
    ...
    const NSInteger networkParseFailureErrorCode = [BBCSMPErrorCodes initialLoadFailed];
...
- (void)preparePlayerForCurrentPlayerItem
{
    ...
    if (_playerItem) {
        ...
        if ([_playerItem.decoder respondsToSelector:@selector(eventBus)]) {
            BBCSMPEventBus* eventBus = _playerItem.decoder.eventBus;
            [_statisticsManager setEventBus:eventBus];
        }
...
{
    [[_playerItem subtitleProvider] setSubtitlesActive:YES];
}

- (void)deactivateSubtitles
{
    [[_playerItem subtitleProvider] setSubtitlesActive:NO];
}
...
#pragma GCC diagnostic pop
        [self.oldToCompatObservers setObject:wrappedObserver forKey:[self idOfObserver:observer]];
    ...
    if ([observer conformsToProtocol:@protocol(BBCSMPSubtitleObserver)]) {
        [_subtileObserverManager addObserver: (id<BBCSMPSubtitleObserver>)observer];
    }
...
{
    if ([observer conformsToProtocol:@protocol(BBCSMPStateObserver)]) {
        NSNumber *key = [self idOfObserver:observer];
    ...
    if ([observer conformsToProtocol:@protocol(BBCSMPSubtitleObserver)]) {
        [_subtileObserverManager removeObserver:(id<BBCSMPSubtitleObserver>)observer];
    }
    if ([observer conformsToProtocol:@protocol(BBCSMPAirplayObserver)]) {
...
- (NSNumber * _Nonnull)idOfObserver:(id<NSObject> _Nonnull)observer {
    return [NSNumber numberWithLong:[observer hash]];
}
...
- (void)setPlayerItem:(id<BBCSMPItem>)playerItem
{
    ...
    if ([_playerItem subtitleProvider] != nil) {
        /// While initializing `SubtitleProvider` in `BBCSMPBackingOffMediaSelectorPlayerItemProvider` there is no access to `BBCSMPPlayerInitialisationContext` where `settingsPersistence` is available. To make use of that we updating the SubtitleProvider here.
        /// If we wanted to move the settings persistence from player builder to provider, public API changes are required.
        [[_playerItem subtitleProvider] updateSettings:[[self playerContext] context].settingsPersistence];
        
        [self addProgressObserver:[_playerItem subtitleProvider]];
        [_subtileObserverManager addObserver:_statisticsManager];
        
        __weak typeof(self) weakSelf = self;
        [_subtileObserverManager attachSubtitleProvider:[_playerItem subtitleProvider] failureCallback:^{
            [weakSelf.playerItemProvider requestNextCaptionsItem:^(id<BBCSMPItem> playerItem) {
                [weakSelf setPlayerItem:playerItem];
                [weakSelf.fsm itemLoadedWithItem:playerItem];
                // TODO: https://jira.dev.bbc.co.uk/browse/MOBILE-8059
                // Use SMPErrorDomain and code TBD
            } failure:^(NSError *error) {
                weakSelf.playerItem.subtitleProvider = nil;
                [[BBCSMPPlayer logger] logMessage:[BBCStringLogMessage messageWithMessage:[NSString stringWithFormat:@"attempt requestNextCaptionsItem: %@", error.description]] logLevel:BBCLogLevelDebug];
            }];
        }];
    }
...
- (void)abandonPlaybackWithError:(BBCSMPError*)error
{
    ...
    [self sendInternalErrorEvent:error];
