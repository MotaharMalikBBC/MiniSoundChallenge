...
/// Reads the current volume from the provider and notifies observers of volume changes.
- (void)forceVolumeUpdate
{
    [self notifyObserversOfVolumeChange: _volumeProvider.currentVolume];
}

/// Reads and notifies observers if subtitle is enabled by the user/client from the settings persistence.
- (void)forceSubtitleStateUpdate
{
    [self notifyObserversForProtocol:@protocol(BBCSMPSubtitleObserver) withBlock:^(id<BBCSMPSubtitleObserver> observer) {
        [observer subtitleActivationChanged:@(_context.settingsPersistence.subtitlesActive)];
    }];
}
...
- (void)didUpdateVolume:(float)volume
{
    ...
    [self notifyObserversOfVolumeChange: volume];
}

- (void)notifyObserversOfVolumeChange:(float) toVolume
{
    [self notifyObserversForProtocol:@protocol(BBCSMPVolumeObserver) withBlock:^(id<BBCSMPVolumeObserver> observer) {
        [observer playerVolumeChanged:@(toVolume)];
    }];
}
