...
@property (nonatomic, strong) BBCMediaConnection* captionsConnection;
...
- (void)requestPlayerItem:(BBCSMPItemProviderSuccess)success failure:(BBCSMPItemProviderFailure)failure
{
    ...
    BBCMediaSelectorRequest* request = [[[[[BBCMediaSelectorRequest alloc] initWithVPID:_vpid] withMediaSet:_mediaSet] withCeiling:_ceiling] withSecureConnectionPreference:mediaSelectorPreference];
...
- (BBCSMPMediaSelectorItem *)playerItemForCurrentConnection
{
    ...
    if(_captionsConnection.href != nil && _captionsConnection.transferFormat != nil) {
        /// Settings persistence should use the one defined in BBCSMPPlayerBuilder. As we don't have access to settings persistence here we going with [BBCSMPSettingsPersistenceFilesystemFactory new] here. However, we can update the settings persistence in [BCSMPPlayer setPlayerItem] where the new playerItem gets updated.
        /// If we wanted to move the settings persistence from player builder to provider, public API changes are required.
        playerItem.subtitleProvider = [BBCSMPSubtitleProviderFactory providerWithUrl:[_captionsConnection href] transferFormat:[_captionsConnection transferFormat] settings:[[BBCSMPSettingsPersistenceFilesystemFactory new] createWithDefaultValue:YES]];
    }
    return playerItem;
}
...
{
    BBCMediaConnectionFilter *filter = [BBCMediaConnectionFilter filter];
    filter = [filter withRequiredTransferFormats:@[@"plain", @"dash"]];
    [[_mediaSelectorResponse itemForCaptions] setConnectionFilter:filter];
    _captionsConnection = [[_mediaSelectorResponse itemForCaptions] selectNextConnectionWithError:nil];
}

- (void)requestFailoverPlayerItem:(BBCSMPItemProviderSuccess)success failure:(BBCSMPItemProviderFailure)failure
{
    NSError *clientError = nil;
    _currentConnection = [_selectedItem selectNextConnectionWithError:&clientError];
    
    if (!_currentConnection) {
        NSError *error = [BBCSMPMediaSelectorErrorTransformer convertMediaSelectorClientErrorToNSError: clientError];
...
- (void)requestNextCaptionsItem:(BBCSMPItemProviderSuccess)success failure:(BBCSMPItemProviderFailure)failure {
    [self updateSupportedCaptionsURL];
    if (_captionsConnection != nil) {
        [_connectionResolver resolvePlayerItem: [self playerItemForCurrentConnection] usingPlayerItemCallback:^(id<BBCSMPItem> _Nonnull item) {
                success(item);
        }];
    } else {
        failure([[NSError alloc] initWithDomain:@"No captions connection found" code:0 userInfo:nil]);
