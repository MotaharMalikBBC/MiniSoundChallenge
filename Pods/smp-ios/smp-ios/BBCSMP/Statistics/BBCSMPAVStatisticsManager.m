...
- (void)stateChanged:(BBCSMPState*)state
{
    ...
    case BBCSMPStateLoadingItem: {
        ...
        [_observerManager notifyObserversForProtocol:@protocol(BBCSMPAVStatisticsConsumer)
                                           withBlock:^(id<BBCSMPAVStatisticsConsumer> avConsumer) {
            if ([avConsumer respondsToSelector:@selector(trackAVPlaybackInitiated)]) {
                [avConsumer trackAVPlaybackInitiated];
            }
        }];
        break;
    }
...
- (void)rateChangedEvent:(BBCSMPAVRateChangedEvent*)event {
    [self playerRateChanged:event.rate];
}

- (void)playerRateChanged:(float)newPlayerRate
{
    if ([_avStatisticsConsumer respondsToSelector:@selector(setPlayerPlaybackSpeedAttribute:)]) {
        [_avStatisticsConsumer setPlayerPlaybackSpeedAttribute:newPlayerRate];
    }
}
...
- (void)errorOccurred:(BBCSMPError*)error
{
    ...
    customParameters[@"errorCode"] = [NSNumber numberWithInteger: error.error.code];
    customParameters[@"recoverable"] = [NSNumber numberWithBool: error.recoverable];
...
- (void)subtitleActivationChanged:(NSNumber*)active
{
    ...
    if ([_avStatisticsConsumer respondsToSelector:@selector(setPlayerIsSubtitledAttribute:)]) {
        [_avStatisticsConsumer setPlayerIsSubtitledAttribute: self.subtitlesActive];
    }
    __weak typeof(self) weakSelf = self;
    [_observerManager notifyObserversForProtocol:@protocol(BBCSMPAVStatisticsConsumer)
                                       withBlock:^(id<BBCSMPAVStatisticsConsumer> avConsumer) {
...
{
    if ([_avStatisticsConsumer respondsToSelector:@selector(setPlayerVolumeAttribute:)]) {
        [_avStatisticsConsumer setPlayerVolumeAttribute:volume];
    }
}
...
{
    _eventBus = eventBus;
    [_eventBus addTarget:self selector:@selector(rateChangedEvent:) forEventType:[BBCSMPAVRateChangedEvent class]];
}
