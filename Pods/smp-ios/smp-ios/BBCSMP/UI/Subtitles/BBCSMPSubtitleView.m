...
@property (nonatomic, strong) id<BBCSMPSubtitleStyle> subtitleStyle;
@property (nonatomic, strong) BBCSMPSubtitleStyleBuilder styleBuilder;
...
- (instancetype)initWithFrame:(CGRect)frame styleBuilder:(BBCSMPSubtitleStyleBuilder)styleBuilder
{
    if ((self = [super initWithFrame:frame])) {
        [self commonInitWithStyleBuilder: styleBuilder];
    }
    return self;
}

- (void)commonInitWithStyleBuilder:(BBCSMPSubtitleStyleBuilder)styleBuilder
{
...
- (void)updateSubtitleLabelFramesForPositionalSubtitle:(BBCSMPSubtitle*)subtitle withSubtitleLabel:(UILabel*)subtitleLabel
{
    ...
    subtitleLabel.backgroundColor = [_subtitleStyle backgroundColor];
...
- (NSInteger)calculateNestedFontSizes:(NSString*)parentStyleKey ofSubtitle:(BBCSMPSubtitle*)subtitle
{
    NSMutableDictionary *parentAttributes = [self.styleDictionary objectForKey:parentStyleKey];
    NSString *parentFontSize = [parentAttributes objectForKey:@"fontSize"];
    NSInteger intParentFontSize = [parentFontSize integerValue];
    if (parentFontSize) {
        NSString *childStyleName = [subtitle.styleDictionary objectForKey:@"style"];
        NSMutableDictionary *childAttributes = [self.styleDictionary objectForKey:childStyleName];
        if ([childAttributes objectForKey:@"fontSize"] != nil) {
            NSString *childFontSize = [childAttributes objectForKey:@"fontSize"];
            NSInteger intChildFontSize = [childFontSize integerValue];
            NSInteger calculatedSize = (intParentFontSize * intChildFontSize) * 0.01;
            return calculatedSize;
        }
        return intParentFontSize;
    }
    return nil;
}
...
- (NSDictionary*)attributesForChunk:(BBCSMPSubtitleChunk*)chunk ofSubtitle:(BBCSMPSubtitle*)subtitle
{
    ...
    if (self.baseStyleKey != nil) {
        ...
        for (NSString* baseStyle in baseStyleKeys) {
            ...
            NSDictionary* parentStyleDictionary = [self.styleDictionary objectForKey:baseStyle];
            if ([parentStyleDictionary objectForKey:@"fontSize"]) {
                parentStyleKey = baseStyle;
    ...
    NSInteger calculatedFontSize = [self calculateNestedFontSizes:parentStyleKey ofSubtitle:subtitle];
    if (calculatedFontSize) {
        NSString* styledFontSize = [NSString stringWithFormat:@"%ld%%", calculatedFontSize];
        [mergedStyles setObject:styledFontSize forKey:@"fontSize"];
    }
    
    _subtitleStyle = _styleBuilder(mergedStyles);
    
    [result setObject:[_subtitleStyle textColor] forKey:NSForegroundColorAttributeName];
...
{
    return [_subtitleStyle fontForSize:fontSize isItalic:isItalic];
}
...
- (UIFont*)calculateFontSizeFromLabelHeight:(CGFloat)labelHeight withItalicFont:(BOOL)isItalic
{
    ...
    mid = mid * [_subtitleStyle relativeFontSize];
                               ...
                               appendingIntoAttributes:(NSMutableDictionary *)attributes
{
    ...
    [self processLineSpacingFromStyles:mergedStyles appendingIntoAttributes:attributes];
...
- (void)processLineSpacingFromStyles:(NSMutableDictionary *)mergedStyles
                      appendingIntoAttributes:(NSMutableDictionary *)attributes
{
    NSString* lineHeightString = [mergedStyles objectForKey:@"lineHeight"];
    if (!lineHeightString || [_subtitleStyle fillLineGap] == YES) {
        return;
    }
...
- (void)processFontAttributesFromStyles:(NSMutableDictionary *)mergedStyles
                 ...
                 appendingIntoAttributes:(NSMutableDictionary *)attributes
{
    NSString* fontSizeString = [_subtitleStyle fontSizeAsPercentage];
...
- (void)processBackgroundAttributesFromStyles:(NSMutableDictionary *)mergedStyles
                      appendingIntoAttributes:(NSMutableDictionary *)attributes
{
    UIColor *textHighlightColor = [_subtitleStyle textHighlightColor];
    if (!textHighlightColor) {
        return;
    }
    [attributes setObject:textHighlightColor forKey:NSBackgroundColorAttributeName];
    [attributes addEntriesFromDictionary: [_subtitleStyle textEdgeStyleAttributes]];
}
